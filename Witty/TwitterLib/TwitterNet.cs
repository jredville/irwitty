...
namespace TwitterLib
{
    ...
    public class TwitterNet : IServiceApi
    {
        ...
        public event EventHandler<UserCollectionEventArgs> OnGetFriends;

        public virtual void NotifyGetFriends(UserCollectionEventArgs e)
        {
            EventHandler<UserCollectionEventArgs> temp = OnGetFriends;
        ...
        public void BeginGetFriends(int userId)
        {
            ...
            pagesOfFriends = (friendsCount / 100) + 1;

            string pageRequestUrl = requestURL;

            for (int count = 1; count <= pagesOfFriends; count++)
            {
                pageRequestUrl = requestURL + "?page=" + count;

                // Create the web request
                HttpWebRequest request = CreateTwitterRequest(pageRequestUrl);
                request.BeginGetResponse(EndGetFriends, request);
            }
        }

        private void EndGetFriends(IAsyncResult result)
        {
            HttpWebRequest request = (HttpWebRequest) result.AsyncState;
            string query = request.RequestUri.Query;
            int page = 0;
            Int32.TryParse(query.Substring(query.Length, 1), out page);
            ...
            //TODO: This is a race condition, but I think it will work for proof of concept
            if (page == pagesOfFriends) NotifyGetFriends(new UserCollectionEventArgs(friends));
        }
        ...
        private HttpWebRequest CreateTwitterRequest(string Uri)
        {
            ...
            NetworkCredential credentials = new NetworkCredential(username, TwitterNet.ToInsecureString(password));
    ...
    public class UserCollectionEventArgs : EventArgs
    {
        private readonly UserCollection _users;

        public UserCollection Users { get { return _users; } }

        public UserCollectionEventArgs(UserCollection users)
        {
            _users = users;
        }
    }
